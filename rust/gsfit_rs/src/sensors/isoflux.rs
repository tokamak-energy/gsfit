    ...
    fn greens_with_plasma(&mut self, plasma: PyRef<Plasma>) {
        ...
        for sensor_name in self.results.keys() {
            ...
            for i_time in 0..n_time {
                ...
                let g_d_psi_d_z_location_1: Array2<f64> = greens_calculator.d_psi_d_z(); // shape = [1, n_z * n_r]
                ...
                let g_with_plasma_location_2: Array1<f64> = g_location_2_matrix.sum_axis(Axis(0)); // g_br_full_location_2.shape = [1, n_z * n_r];  g_br_location_2.shape = [n_z * n_r]
                let g_d_plasma_d_z_location_2: Array1<f64> = g_d_psi_d_z_location_2_matrix.sum_axis(Axis(0)); // shape = n_r * n_z

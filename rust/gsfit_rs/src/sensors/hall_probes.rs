...
impl HallProbes {
    ...
    pub fn split_into_static_and_dynamic(&mut self, times_to_reconstruct: &Array1<f64>) -> (Vec<SensorsStatic>, Vec<SensorsDynamic>) {
        ...
        // Create the `HallProbesStatic` data
        ...
        for i_sensor in 0..n_sensors_all {
            ...
            let interpolator = interpolator::Dim1Linear::new(&time_experimental, &measured_experimental)
                .expect("HallProbes.split_into_static_and_dynamic: Can't make interpolator");

            // Do the interpolation
            let measured_this_coil: Array1<f64> = interpolator.interp_array(&times_to_reconstruct)
                .expect("HallProbes.split_into_static_and_dynamic: Can't do interpolation");
    ...
    pub fn greens_with_coils_rs(&mut self, coils: Coils) {
        for sensor_name in self.results.keys() {
            ...
            for pf_coil_name in coils.results.get("pf").keys() {
                ...
                // g = sqrt(b_r^2 + b_z^2 + b_phi^2) = sqrt((g_r*I_PF)^2 + (g_z*I_PF)^2 + (I_rod/R)^2)
                ...
                self.results
                    ...
                    .get_or_insert("br")
                    .get_or_insert("pf")
                    .insert(&pf_coil_name, g_br);
                self.results
                    .get_or_insert(&sensor_name)
                    .get_or_insert("greens")
                    .get_or_insert("bz")
                    .get_or_insert("pf")
                    .insert(&pf_coil_name, g_bz);
            }
        }
    }
    ...
    pub fn greens_with_plasma_rs(&mut self, plasma: Plasma) {
        ...
        for sensor_name in self.results.keys() {
            ...
            self.results
                ...
                .get_or_insert("greens")
                .get_or_insert("br")
                .insert("plasma", g_br); // shape = [[n_z * n_r]]
            self.results
                .get_or_insert(&sensor_name)
                .get_or_insert("greens")
                .get_or_insert("bz")
                .insert("plasma", g_bz); // shape = [[n_z * n_r]]
            ...
            self.results
                ...
                .get_or_insert("greens")
                .get_or_insert("br")
                .insert("d_plasma_d_z", g_d_plasma_br_d_z);
            self.results
                .get_or_insert(&sensor_name)
                .get_or_insert("greens")
                .get_or_insert("bz")
                .insert("d_plasma_d_z", g_d_plasma_bz_d_z);
        }
    }
    ...
    pub fn greens_with_passives_rs(&mut self, passives: Passives) {
        ...
        for sensor_name in self.results.keys() {
            ...
            for passive_name in passives.results.keys() {
                ...
                for dof_name in dof_names {
                    let (g_br_full, g_bz_full): (Array2<f64>, Array2<f64>) = greens_d_b_d_z(

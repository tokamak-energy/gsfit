...
pub fn marching_squares(
    ...
    println!("r_xpt={}; z_xpt={};", xpt_r, xpt_z);
    ...
    // println!("boundary_contour.r={:?};", boundary_contour.r);
    // println!("boundary_contour.z={:?};", boundary_contour.z);
...
// /// preventing self-crossings. This version:
// /// - starts at first_and_last_boundary_points[0]
// /// - connects points by nearest-neighbour BUT rejects any next step that
// ///   would cause a self-intersection with the path built so far
// /// - defers connecting to the last boundary point until all interior points
// ///   are consumed, then inserts it in a place that avoids crossings (or
// ///   minimally increases length as a fallback)
...
//     // Endpoints determined near the X-point
//     if first_and_last_boundary_points.len() < 2 {
//         // Degenerate: nothing to connect; just return the X-point twice
//         return (vec![xpt_r, xpt_r], vec![xpt_z, xpt_z]);
//     }
//     let start = first_and_last_boundary_points[0];
//     let end = first_and_last_boundary_points[1];
...
//     // If there are no interior points, just return the trivial chain xpt -> start -> end -> xpt
//     if interior.is_empty() {
//         return (vec![xpt_r, start.0, end.0, xpt_r], vec![xpt_z, start.1, end.1, xpt_z]);
//     }

//     // Helper: would adding segment (r_last,z_last)->(r_new,z_new) cross the existing polyline?
//     fn would_cross(r_path: &Vec<f64>, z_path: &Vec<f64>, r_last: f64, z_last: f64, r_new: f64, z_new: f64) -> bool {
//         if r_path.len() < 2 {
//             return false;
//         }
//         // Existing segments are (r_path[i],z_path[i]) -> (r_path[i+1],z_path[i+1])
//         // Skip the very last segment because it shares the r_last,z_last endpoint
...
//     // Start the ordered path with X-point and the chosen start point
//     let mut r_sorted: Vec<f64> = vec![xpt_r, start.0];
//     let mut z_sorted: Vec<f64> = vec![xpt_z, start.1];

//     // Track which interior points have been used
...
//     // Iteratively add nearest interior point that does not create a crossing
//     while n_used < interior.len() {
//         // Build candidate list sorted by distance from current point
...
//         // If every candidate would cross, relax and take the nearest unused (to guarantee progress)
//         if chosen.is_none() {
//             // Find the nearest unused
//             let mut best_i = None;
...
//             cur = p;
//         } else {
//             // No progress possible (shouldn't happen) – break to avoid infinite loop
//             break;
//         }
//     }

//     // Try to append the end point without causing a crossing
//     let mut can_append_end = !would_cross(&r_sorted, &z_sorted, cur.0, cur.1, end.0, end.1);

//     if can_append_end {
//         r_sorted.push(end.0);
//         z_sorted.push(end.1);
//     } else {
//         // Insert end between an existing segment where it won't cross. Choose position
//         // that minimizes added length as a tie-breaker.
//         let m = r_sorted.len();
//         let mut best_pos: Option<(usize, f64)> = None; // (insert_index, added_length)
//         for k in 0..(m - 1) {
//             // Check two new segments: k->end and end->k+1 against all other segments
//             let mut crosses = false;
//             for i in 0..(m - 1) {
//                 if i == k {
//                     continue; // skip the segment being replaced
//                 }
//                 // k -> end vs segment i
...
//                     break;
//                 }
//                 // end -> k+1 vs segment i
//                 if segments_intersect(r_sorted[i], z_sorted[i], r_sorted[i + 1], z_sorted[i + 1], end.0, end.1, r_sorted[k + 1], z_sorted[k + 1]) {
//                     crosses = true;
//                     break;
//                 }
//             }
//             if !crosses {
//                 // Compute added length
...
//         } else {
//             // Last resort: append anyway to avoid returning only X-point and one other
//             r_sorted.push(end.0);
//             z_sorted.push(end.1);
//         }
//     }
...
// /// - Classify points by the sign of the 2D cross product with the vector
// ///   from X-point to magnetic axis (left/right of that line).
// /// - First step: only allow candidates on the SAME side as the chosen start
// ///   (this disambiguates the two branches at the X-point).
// /// - Afterwards: nearest-neighbour with no-cross guard, as before.
// /// - Returns [xpt, start, ..., end, xpt].
// pub fn sort_boundary_points_version_2(
//     unsorted_boundary_points: HashMap<(usize, usize, usize, usize), (f64, f64)>,
//     xpt_r: f64,
...
//     // Left/right classifier with respect to the line from X-point to magnetic axis
//     #[inline]
//     fn side_of_axis(xr: f64, xz: f64, vr: f64, vz: f64, p: (f64, f64)) -> i8 {
//         // cross = v x (p - x) = vr*(pz-xz) - vz*(pr-xr)
...
//     // Direction from x-point to magnetic axis
//     let vr = mag_r - xpt_r;
//     let vz = mag_z - xpt_z;
//     let v_norm = (vr * vr + vz * vz).sqrt();

//     // Guard against degenerate input
//     let (vr, vz) = if v_norm > 0.0 { (vr / v_norm, vz / v_norm) } else { (1.0, 0.0) };

//     // Ensure start and end are on different sides if possible by swapping
...
//         // Swap to try the other ordering (caller passes arbitrary order)
...
//     // Classify interior points by side once (0 means on the line; treat as both)
//     let interior_side: Vec<i8> = interior.iter().map(|&p| side_of_axis(xpt_r, xpt_z, vr, vz, p)).collect();

//     // First step: restrict to the same side as 'start' to disambiguate the branch near X-point
//     let preferred_side = side_of_axis(xpt_r, xpt_z, vr, vz, start);

//     let mut first_step_done = false;

//     // Iteratively add nearest interior point with a no-cross guard
//     while n_used < interior.len() {
//         // Build candidate list sorted by distance from current point
//         let mut cand: Vec<(usize, f64)> = interior
//             .iter()
...
//         // If we haven't taken the first step yet, keep only points on the preferred side (or on the line)
//         if !first_step_done && preferred_side != 0 {
//             cand.retain(|(i, _)| interior_side[*i] == preferred_side || interior_side[*i] == 0);
//             // If that leaves nothing, fall back to all candidates
//             if cand.is_empty() {
//                 // no-op; we'll use full candidate list below by rebuilding it
//                 cand = interior
//                     .iter()
//                     .enumerate()
//                     .filter(|(i, _)| !used[*i])

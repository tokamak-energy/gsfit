...
pub fn find_stationary_points_using_winding_number(
    ...
    // This protects against a sneaky bug where `d_psi_d_z_2d` is exactly zero at grid points when there is perfect up/down symmetry.
    // TODO: I don't like this fix...
    let mut d_psi_d_z_2d: Array2<f64> = d_psi_d_z_2d.to_owned();
    let mut d_psi_d_r_2d: Array2<f64> = d_psi_d_r_2d.to_owned();
    for i_r in 0..n_r {
        for i_z in 0..n_z {
...
/// In this test `psi` is perfectly up/down symmetric, so `d_psi_d_z` is exactly zero at Z=0.
/// `d_psi_d_z = 0` is degenerate (all values of R at Z=0 are valid), so `cubic_interpolation` doesn't record any `d_psi_d_z = 0` events.
#[test]
fn test_3_find_stationary_points_using_winding_number_with_up_down_symmetry() {
    ...
    let mut found_magnetic_axis: bool = false;
    'stationary_points_loop: for stationary_point in stationary_points {
        if (0.78..=0.84).contains(&stationary_point.r) && (-0.01..=0.01).contains(&stationary_point.z) {
            found_magnetic_axis = true;
            break 'stationary_points_loop;
        }
    }

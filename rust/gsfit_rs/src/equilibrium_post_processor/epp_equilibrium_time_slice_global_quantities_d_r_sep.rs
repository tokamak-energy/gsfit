//! `time_slice(itime)/global_quantities/d_r_sep`
...
/// in the time-slice.
///
/// `d_r_sep` is the distance, at the height of the magnetic axis and on the outboard side, between
/// the flux surface through the lower X-point and the one through the upper X-point:
///
/// ```text
/// d_r_sep = r_outboard(psi_x_lower) - r_outboard(psi_x_upper)
/// ```
///
/// The active X-point is whichever of the two bounds the plasma, so its surface is the innermost of
/// the pair. A lower single null therefore gives a negative `d_r_sep` and an upper single null a
...
///
/// # Arguments
/// * `time_slice` - the solved time-slice; `global_quantities/d_r_sep` is written into it
...
/// converge.
///
/// **Must run after `epp_equilibrium_time_slice_profiles_1d_r_inboard_and_r_outboard`**, which
/// supplies the outboard midplane radius of the separatrix.
pub fn epp_equilibrium_time_slice_global_quantities_d_r_sep(time_slice: &mut EquilibriumTimeSlice, wall_ids: &WallIds) {
    time_slice.global_quantities.d_r_sep = Some(f64::NAN);
    ...
    let d_r_sep: f64 = (psi_x_lower - psi_x_upper) / d_psi_d_r_omp;

    time_slice.global_quantities.d_r_sep = Some(d_r_sep);
}

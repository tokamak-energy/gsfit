...
pub fn calculate(time_slice: &mut EquilibriumTimeSlice, _constant_values: &ConstantValues, _intermediate_values: &mut IntermediateValues) {
    ...
    let rho_tor_profile: &Array1<f64> = time_slice.profiles_1d.rho_tor.as_ref().unwrap();

    time_slice.boundary.psi_norm = psi_norm_profile.last().copied();
    time_slice.boundary.rho_tor = rho_tor_profile.last().copied();
}
...
mod tests {
    ...
    fn boundary_values_are_the_profile_endpoints() {
        ...
        time_slice.profiles_1d.psi_norm = Some(array![0.0, 0.4, 1.0]);
        time_slice.profiles_1d.rho_tor = Some(array![0.0, 0.2, 0.5]);

        calculate(&mut time_slice, &constant_values_for_test(), &mut intermediate_values_for_test());

        assert_abs_diff_eq!(time_slice.boundary.psi_norm.unwrap(), 1.0, epsilon = 1e-15);
        assert_abs_diff_eq!(time_slice.boundary.rho_tor.unwrap(), 0.5, epsilon = 1e-15);
    }
}

...
pub fn calculate(time_slice: &mut EquilibriumTimeSlice, _constant_values: &ConstantValues, _intermediate_values: &mut IntermediateValues) {
    ...
    let rho_tor_norm_profile: &Array1<f64> = &time_slice.profiles_1d.rho_tor_norm;
    ...
    time_slice.global_quantities.q_min.psi = q_min_psi;
    time_slice.global_quantities.q_min.psi_norm = q_min_psi_norm;
    time_slice.global_quantities.q_min.rho_tor_norm = q_min_rho_tor_norm;
    time_slice.global_quantities.q_min.value = q_min_value;
}
...
mod tests {
    ...
    fn minimum_and_coordinates_come_from_the_same_finite_profile_point() {
        ...
        time_slice.profiles_1d.psi = array![3.0, 2.5, 2.0, 1.5, 1.0];
        time_slice.profiles_1d.psi_norm = array![0.0, 0.25, 0.5, 0.75, 1.0];
        time_slice.profiles_1d.q = array![2.0, f64::NAN, 1.4, 1.6, f64::NAN];
        time_slice.profiles_1d.rho_tor_norm = array![0.0, 0.4, 0.65, 0.85, 1.0];

        calculate(&mut time_slice, &constant_values_for_test(), &mut intermediate_values_for_test());

        assert_abs_diff_eq!(time_slice.global_quantities.q_min.psi, 2.0, epsilon = 1e-15);
        assert_abs_diff_eq!(time_slice.global_quantities.q_min.psi_norm, 0.5, epsilon = 1e-15);
        assert_abs_diff_eq!(time_slice.global_quantities.q_min.rho_tor_norm, 0.65, epsilon = 1e-15);
        assert_abs_diff_eq!(time_slice.global_quantities.q_min.value, 1.4, epsilon = 1e-15);
    }

    #[test]
    fn all_outputs_are_nan_when_q_has_no_finite_value() {
        let mut time_slice: EquilibriumTimeSlice = EquilibriumTimeSlice::default();
        time_slice.profiles_1d.psi = array![f64::NAN, f64::NAN];
        time_slice.profiles_1d.psi_norm = array![0.0, 1.0];
        time_slice.profiles_1d.q = array![f64::NAN, f64::NAN];
        time_slice.profiles_1d.rho_tor_norm = array![f64::NAN, f64::NAN];

        calculate(&mut time_slice, &constant_values_for_test(), &mut intermediate_values_for_test());

        assert!(time_slice.global_quantities.q_min.psi.is_nan());
        assert!(time_slice.global_quantities.q_min.psi_norm.is_nan());
        assert!(time_slice.global_quantities.q_min.rho_tor_norm.is_nan());
        assert!(time_slice.global_quantities.q_min.value.is_nan());
    }
}

...
pub fn calculate(time_slice: &mut EquilibriumTimeSlice, _constant_values: &ConstantValues, _intermediate_values: &mut IntermediateValues) {
    ...
    time_slice.global_quantities.magnetic_axis.b_field_phi = f_profile[0] / mag_r;
}
...
mod tests {
    ...
    fn magnetic_axis_field_retains_the_sign_of_f() {
        ...
        time_slice.global_quantities.magnetic_axis.r = 0.8;
        time_slice.profiles_1d.f = array![-0.48, -0.47];

        calculate(&mut time_slice, &constant_values_for_test(), &mut intermediate_values_for_test());

        assert_abs_diff_eq!(time_slice.global_quantities.magnetic_axis.b_field_phi, -0.6, epsilon = 1e-15);
    }
}

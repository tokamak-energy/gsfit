...
pub fn read_npy_0d(path: &Path) -> f64 {
    ...
    assert_eq!(shape.len(), 1, "npy_reader: Expected a 0D (scalar) array, got shape {:?}", shape);
    assert_eq!(shape[0], 1, "npy_reader: Expected a scalar (shape (1,)), got shape {:?}", shape);
...
pub fn write_npy_0d(path: &Path, value: f64) {
    let shape_str: String = String::from("(1,)");
...
fn build_npy_header(shape_str: &str) -> Vec<u8> {
    ...
    let padded_total: usize = ((unpadded_len + 63) / 64) * 64;
...
mod tests {
    ...
    fn test_write_and_read_npy_1d() {
        let file_path: std::path::PathBuf = std::env::temp_dir().join("gsfit_testing_npy_writer_1d.npy");
    ...
    fn test_write_and_read_npy_2d() {
        let file_path: std::path::PathBuf = std::env::temp_dir().join("gsfit_testing_npy_writer_2d.npy");
    ...
    fn test_write_and_read_npy_0d() {
        let file_path: std::path::PathBuf = std::env::temp_dir().join("gsfit_testing_npy_writer_0d.npy");
    ...
    fn test_write_npy_1d_single_element() {
        let file_path: std::path::PathBuf = std::env::temp_dir().join("gsfit_testing_npy_writer_1d_single.npy");
    ...
    fn test_write_npy_2d_single_row() {
        let file_path: std::path::PathBuf = std::env::temp_dir().join("gsfit_testing_npy_writer_2d_single_row.npy");

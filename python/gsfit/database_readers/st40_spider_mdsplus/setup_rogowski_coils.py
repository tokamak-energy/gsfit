...
def setup_rogowski_coils(
    ...
    **This method is specific to ST40's SPIDER stored on MDSplus.**
    ...
    # SPIDER rogowki_coils from MDSplus
    time = conn.get(f"\\SPIDER::TOP.{spider_run_name}:TIME").data().astype(np.float64)
    measurements = conn.get(f"\\SPIDER::TOP.{spider_run_name}.ROG.ALL:I").data().astype(np.float64)
    names = conn.get(f"\\SPIDER::TOP.{spider_run_name}.ROG.ALL:NAME").data().astype(str)

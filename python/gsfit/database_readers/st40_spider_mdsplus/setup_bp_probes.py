...
def setup_bp_probes(
    ...
    **This method is specific to ST40's SPIDER stored on MDSplus.**
    ...
    measurements = conn.get(f"\\SPIDER::TOP.{spider_run_name}.BPPROBE.ALL:B").data().astype(np.float64)
    names = conn.get(f"\\SPIDER::TOP.{spider_run_name}.BPPROBE.ALL:NAME").data().astype(str)
    ...
    # names_long = conn.get(f"\\SPIDER::TOP.{spider_run_name}.BPPROBE.ALL:NAME").data().astype(str)
    # sensors_names = np.char.replace(names_long, "BPPROBE_", "P")
    # sensors_r = conn.get(f"\\SPIDER::TOP.{spider_run_name}.BPPROBE.ALL:R").data().astype(np.float64)
    # sensors_z = conn.get(f"\\SPIDER::TOP.{spider_run_name}.BPPROBE.ALL:Z").data().astype(np.float64)
    # sensors_angle_pol = conn.get(f"\\SPIDER::TOP.{spider_run_name}.BPPROBE.ALL:ANGLE").data().astype(np.float64)

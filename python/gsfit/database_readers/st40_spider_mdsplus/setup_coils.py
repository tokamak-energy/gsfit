...
# # The PF coil's are defined in the `P2.3/coil.dat` file, in the `SPIDER` GitLab repository.
# # On `smaug2`, this file is at `/home/alexei.dnestrovskij/ASTRA_TE_user/astra_user/exp/equ/P2.3/coil.dat`
# spider_coil_description = dict(
#     DIVT=dict(rc=0.1930, zc=0.7907, wc=0.0566, hc=0.1351, ndi=20),
#     DIVB=dict(rc=0.1930, zc=-0.7907, wc=0.0566, hc=0.1351, ndi=20),
#     BVLT=dict(rc=1.3860, zc=0.422, wc=0.11, hc=0.1156, ndi=12),
#     BVLB=dict(rc=1.3860, zc=-0.422, wc=0.11, hc=0.1156, ndi=12),
#     BVUT=dict(rc=1.0470, zc=1.0390, wc=0.0552, hc=0.1382, ndi=20),
#     BVUB=dict(rc=1.0470, zc=-1.0390, wc=0.0552, hc=0.1382, ndi=20),
#     SOL=dict(rc=0.1295, zc=0.0000, wc=0.0161, hc=1.4711, ndi=64),
#     MCT=dict(rc=0.7500, zc=0.3550, wc=0.0407, hc=0.0467, ndi=12),
#     MCB=dict(rc=0.7500, zc=-0.3550, wc=0.0407, hc=0.0467, ndi=12),
#     PSHT1=dict(rc=0.8518, zc=0.669, wc=0.007, hc=0.0232, ndi=4),
#     PSHB1=dict(rc=0.8518, zc=-0.669, wc=0.007, hc=0.0232, ndi=4),
#     PSHT2=dict(rc=0.833, zc=0.7016, wc=0.007, hc=0.0232, ndi=4),
#     PSHB2=dict(rc=0.833, zc=-0.7016, wc=0.007, hc=0.0232, ndi=4),
# )
...
# Taken from `https://tokamak-devlin.tokamak.local/gitlab/physics/SPIDER/-/blob/main/EFIT/coil.dat?ref_type=heads`
spider_coil_description = dict(
    DIVT=dict(rc=0.1930, zc=0.7907, wc=0.0566, hc=0.1351, ndi=20),
    DIVB=dict(rc=0.1930, zc=-0.7907, wc=0.0566, hc=0.1351, ndi=20),
    BVLT=dict(rc=1.3860, zc=0.408, wc=0.11, hc=0.1156, ndi=12),
    BVLB=dict(rc=1.3860, zc=-0.43, wc=0.11, hc=0.1156, ndi=12),
    BVUT=dict(rc=1.0470, zc=1.0390, wc=0.0552, hc=0.1382, ndi=20),
    BVUB=dict(rc=1.0470, zc=-1.0390, wc=0.0552, hc=0.1382, ndi=20),
    SOL=dict(rc=0.1295, zc=0.0000, wc=0.0161, hc=1.4711, ndi=64),
    MCT=dict(rc=0.7500, zc=0.3550, wc=0.0407, hc=0.0467, ndi=12),
...
# 20
# 0.1930   0.7907  0.0566  0.1351    0.0       90.0  0.0  28       1              DIVT 1
# 20
# 0.1930  -0.7907  0.0566  0.1351    0.0       90.0  0.0  28       1              DIVB 2
# 12
# 1.3860   0.422  0.11  0.1156    0.0       90.0  0.0  16       2              BVLT 3
# 12
# 1.3860  -0.422  0.11  0.1156    0.0       90.0  0.0  16       2              BVLB 4
# 20
# 1.0470   1.0390  0.0552  0.1382    0.0       90.0  0.0  24       3              BVUT 5
# 20
# 1.0470  -1.0390  0.0552  0.1382    0.0       90.0  0.0  24       4              BVUB 6
# 64
# 0.1295   0.0000  0.0161  1.4711    0.0       90.0  0.0  190      5              CS  7
# 12
# 0.7500   0.3550  0.0407  0.0467    0.0       90.0  0.0  11       6              MCT  8
# 12
# 0.7500  -0.3550  0.0407  0.0467    0.0       90.0  0.0  11       6              MCB  9
# 4
# 0.8518   0.669  0.007  0.0232    0.0      120.0  0.0   4       7              PSHT1 10
# 4
# 0.8518  -0.669  0.007  0.0232    0.0       60.0  0.0   4       7              PSHB1 11
# 4
# 0.833   0.7016  0.007  0.0232    0.0      120.0  0.0   4       7              PSHT2 12
# 4
# 0.833  -0.7016  0.007  0.0232    0.0       60.0  0.0   4       7              PSHB2 13


def dividing_parallograms(coil_dictionary) -> tuple[int, int]:
    """
    The PF coils are described as parallograms, and discretized in `SPIDER/SRC/Trecur_2.f`, in the `equil_spider_tepm` GitLab repository.
...
def setup_coils(
    ...
    **This method is specific to ST40's SPIDER stored on MDSplus.**
    ...
    # Extract the spider_run_name from settings
    ...
    # PF coil currents
    # ["IPL", "MC", "PSH", "DIV", "BVL", "BVUT", "BVUB", "CS"]
    # [0,     1,    2,     3,     4,     5,      6,      7]
    currents = conn.get(f"\\SPIDER::TOP.{spider_run_name}.COILS.PSU2PF:I").data().astype(np.float64) * 1.0e6

    # Add BVLB PF coil
    coil_name = "BVLB"
    coil_r, coil_z = calculate_coil_filament_positions(spider_coil_description[coil_name])
    ...
    coil_name = "MCT"
    coil_r, coil_z = calculate_coil_filament_positions(spider_coil_description[coil_name])
    currents_local = currents[:, 1] * 11.0 / len(coil_r)
    coils.add_pf_coil(
        coil_name,
        coil_r,
        coil_z,

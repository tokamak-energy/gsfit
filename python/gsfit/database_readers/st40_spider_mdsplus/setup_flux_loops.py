...
def setup_flux_loops(
    ...
    **This method is specific to ST40's SPIDER stored on MDSplus.**
    ...
    measurements = conn.get(f"\\SPIDER::TOP.{spider_run_name}.FLOOP.ALL:PSI").data().astype(np.float64)
    names = conn.get(f"\\SPIDER::TOP.{spider_run_name}.FLOOP.ALL:NAME").data().astype(str)

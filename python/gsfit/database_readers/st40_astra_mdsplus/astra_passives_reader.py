...
def astra_passives_reader() -> dict[str, dict[str, Any]]:
    ...
    filaments: dict[str, dict[str, list[float]]] = defaultdict(lambda: {"r": [], "z": [], "d_r": [], "d_z": [], "resistivity": []})
    ...
    with open(fires_dat_path, "r") as f:
        for line in f:
            ...
            passive_r = _parse_fortran_float(parts[1])
            passive_z = _parse_fortran_float(parts[2])
            passive_d_r = _parse_fortran_float(parts[3])
            passive_d_z = _parse_fortran_float(parts[4])
            passive_resistivity = _parse_fortran_float(parts[5])
            name = parts[7].strip()

            filaments[name]["r"].append(passive_r)
            filaments[name]["z"].append(passive_z)
            filaments[name]["d_r"].append(passive_d_r)
            filaments[name]["d_z"].append(passive_d_z)
            filaments[name]["resistivity"].append(passive_resistivity)
    ...
    for passive_name, data in filaments.items():
        r = np.array(data["r"]).astype(np.float64)
        z = np.array(data["z"]).astype(np.float64)
        d_r = np.array(data["d_r"]).astype(np.float64)
        d_z = np.array(data["d_z"]).astype(np.float64)
        n_filaments: int = r.size
        ...
        angle_1 = np.zeros(n_filaments, dtype=np.float64)
        angle_2 = np.zeros(n_filaments, dtype=np.float64)

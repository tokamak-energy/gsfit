...
def plot(gsfit_controller: Gsfit, ax: matplotlib.axes.Axes, time_desired: float, color: str = "blue", linestyle: str = "dashed") -> None:
    ax.set_aspect("equal")
    ...
    psi_levels = np.linspace(np.min(gsfit_psi), np.max(gsfit_psi), 35)
    ax.contour(gsfit_r, gsfit_z, gsfit_psi, levels=psi_levels, colors=color, linewidths=0.6, linestyles=linestyle)
    ax.plot(gsfit_boundary_r, gsfit_boundary_z, color=color, linestyle="solid", label="GSFit", linewidth=1.0)

import matplotlib.axes
import numpy as np
from gsfit_rs.imas import equilibrium_paths as ep
from scipy.constants import mu_0

from ..gsfit import Gsfit


def plot(gsfit_controller: Gsfit, ax: matplotlib.axes.Axes, time_desired: float, color: str = "blue", linestyle: str = "solid") -> None:
    plasma = gsfit_controller.plasma

    time = plasma.get(ep.time_slice[:].time)
    i_time = int(np.argmin(np.abs(time - time_desired)))

    psi_n = plasma.get(ep.time_slice[0].profiles_1d.psi_norm)
    ff_prime = np.asarray(plasma.get(ep.time_slice[i_time].profiles_1d.f_df_dpsi)) / mu_0

    ax.plot(psi_n, ff_prime, color=color, linestyle=linestyle, label="GSFit", linewidth=1.0)
